...
import {
  ROWS, COLS, RED, BLACK, PIECE, PIECE_NAMES,
  createPiece, opponent, initBoard as createBoard,
  getLegalMoves, isInCheck, hasAnyLegalMove, findGeneral,
  uciToMove
} from './board.js';
import { initEngine, getBestMove, destroyEngine, stopEngine } from './ai.js';
import { analyzePosition } from './analysis-engine.js';
import { hostSession, connectGuest, acceptOffer, disconnect, sendMove, sendResign,
...
export function isAiThinking() { return aiThinking; }

// 从开始界面初始化游戏
export function initGame(mode, settings) {
  gameMode = mode || 'pvp';
  if (settings) {
...
// ============ 事件处理 ============
export function onPieceClick(row, col) {
...
export function onCellClick(row, col) {
  if (isEditing) { editPlacePiece(row, col); return; }
...
export function startPostGameAnalysis() {
...
export function goToAnalysisMove(idx) {
...
export function goToAnalysisPrev() {
  var next = analysisMoveIndex - 1;
  if (next < -1) next = -1;
  goToAnalysisMove(next);
}

export function goToAnalysisNext() {
  var next = analysisMoveIndex + 1;
  if (next >= moveHistory.length) next = moveHistory.length - 1;
  goToAnalysisMove(next);
}

export function closePostGameAnalysis() {
...
export function editSelectPiece(type, color) {
...
export function editPlacePiece(row, col) {
  if (!isEditing) return;
...
export function clearEditBoard() {
  if (!isEditing) return;
  for (var r = 0; r < ROWS; r++) for (var c = 0; c < COLS; c++) board[r][c] = null;
  render();
}

export function confirmCustomGame() {
  if (!isEditing) return;
  // 验证：双方各至少有一个将/帅
...
export async function acceptOnlineOffer(sdp) {
  var norm = await normalizeSDP(sdp);
  acceptOffer(norm, null);
}

export async function joinOnlineSession(sdp) {
  var norm = await normalizeSDP(sdp);
  connectGuest(norm);
}
...
export function getOnlineColor() { return onlineColor; }
